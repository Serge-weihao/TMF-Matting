...
def generate_json(data_root, source_bg_dir, composite, nproc, mode):
    ...
    if mode == 'training':
        ...
        num_bg = 1  # each training fg is composited with 100 bg
        fg_dirs = [
    ...
    print(len(name_with_postfix),len(source_bg_paths))
    '''
    ...
    '''
    Dict={}
    for i,args in enumerate(list(zip(name_with_postfix, source_bg_paths, repeat_infos, constants))):
        data_infos = get_data_info(args)
        Dict[str(i)]=data_infos
        mmcv.dump(Dict, osp.join(data_root, save_json_path))
