...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.activation = build_activation_layer(act_cfg) if act_cfg['type']!='ReLU' else nn.ReLU(True)
        self.stride = stride
...
@COMPONENTS.register_module()
class ResLocalHOP_PosEmb(ResShortcutEnc):
    """ResNet backbone with shortcut connection and gca module.
    ...
    * gca module also requires unknown tensor generated by trimap which is \
    ...
    def __init__(self,
        ...
        guidance_in_channels = [3, 16, 32,64]
        guidance_out_channels = [16, 32, 64,64]

        '''
        self.guidance_head = []
        for in_channels, out_channels in zip(guidance_in_channels,
                                             guidance_out_channels):
            self.guidance_head.append(
                ConvModule(
                    in_channels,
                    out_channels,
                    3,
        ...
        #'''
        self.guidance_head1 = ConvModule(
                    guidance_in_channels[0],
                    guidance_out_channels[0],
                    3,
                    stride=2,
                    padding=1,
                    norm_cfg=norm_cfg,
                    ...
                    padding_mode='reflect',
                    order=order)
        self.guidance_head2 = ConvModule(
                    guidance_in_channels[1],
                    guidance_out_channels[1],
                    3,
                    stride=2,
                    padding=1,
                    norm_cfg=norm_cfg,
                    ...
                    padding_mode='reflect',
                    order=order)
        self.guidance_head3 = ConvModule(
                    guidance_in_channels[2],
                    guidance_out_channels[2],
                    3,
                    stride=2,
                    padding=1,
                    norm_cfg=norm_cfg,
                    ...
                    padding_mode='reflect',
                    order=order)
        self.guidance_head4 = ConvModule(
                    guidance_in_channels[3],
                    guidance_out_channels[3],
                    3,
                    stride=2,
                    padding=1,
                    norm_cfg=norm_cfg,
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            super(ResLocalHOP_PosEmb, self).init_weights()
        else:
            raise TypeError('"pretrained" must be a str or None. '
                            f'But received {type(pretrained)}.')
    ...
    def forward(self, x):
        ...
        im_fea4 = self.guidance_head4(im_fea3)
        if self.trimap_channels == 3:
            unknown = x[:, 4:5, ...]
        ...
        return {'out':out, 'mid_fea':{'shortcut': (feat1, feat2, feat3, feat4, feat5),
