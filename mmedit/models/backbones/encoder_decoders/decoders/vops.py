...
_spdynamic_kernel = kernel_loop + '''
extern "C"
__global__ void spdynamic_forward_kernel(
...
_spdynamic_kernel_backward_grad_input = kernel_loop + '''
extern "C"
__global__ void spdynamic_backward_grad_input_kernel(
...
_spdynamic_kernel_backward_grad_weight = kernel_loop + '''
extern "C"
__global__ void spdynamic_backward_grad_weight_kernel(
...
class _spdynamic(Function):
    ...
    def forward(ctx, input, weight, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('spdynamic_forward_kernel', _spdynamic_kernel, Dtype=Dtype(input), nthreads=n,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                f = load_kernel('spdynamic_backward_grad_input_kernel',
                                _spdynamic_kernel_backward_grad_input, **opt)
                f(block=(CUDA_NUM_THREADS,1,1),
                  grid=(GET_BLOCKS(n),1,1),
            ...
            if ctx.needs_input_grad[1]:
                ...
                f = load_kernel('spdynamic_backward_grad_weight_kernel',
                                _spdynamic_kernel_backward_grad_weight, **opt)
                f(block=(CUDA_NUM_THREADS,1,1),
                  grid=(GET_BLOCKS(n),1,1),
...
def _spdynamic_cuda(input, weight, bias=None, stride=1, padding=0, dilation=1):
    """ spdynamic kernel
    """
    ...
    if input.is_cuda:
        out = _spdynamic.apply(input, weight, _pair(stride), _pair(padding), _pair(dilation))
        if bias is not None:
