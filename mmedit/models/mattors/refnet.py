...
class RefNet(BaseMattor):
    ...
    def __init__(self,
                 ...
                 loss_comp=None,
                 gloss_comp=None):
        super(RefNet, self).__init__(backbone, None, train_cfg, test_cfg,
                                       pretrained)
    ...
    def forward_train(self, merged, trimap, meta, alpha, ori_merged, fg, bg):
        ...
        """
        #print((merged, trimap))
        pred_alpha, os4 ,os16 = self.backbone(torch.cat((merged, trimap), 1))

        losses = dict()
        weight = get_unknown_tensor(trimap, meta)
        if self.loss_alpha is not None:
            losses['loss_alpha'] = 0.6*self.loss_alpha(pred_alpha, alpha, weight)+ 0.25*self.loss_alpha(os4, alpha, weight)+ 0.15*self.loss_alpha(os16, alpha, weight)
    ...
    def forward_test(self,
        ...
        """
        pred_alpha,_,_ = self.backbone(torch.cat((merged, trimap), 1))
