...
class IndexNetPM(BaseMattor):
    ...
    def __init__(self,
        ...
        super(IndexNetPM, self).__init__(backbone, None, train_cfg, test_cfg,
                                       pretrained)

        self.loss_alpha = (
            build_loss(loss_alpha) if loss_alpha is not None else None)
        self.aux_alpha = (
            build_loss(aux_alpha) if aux_alpha is not None else None)
        self.loss_comp = (
    ...
    def forward_train(self, merged, trimap, meta, alpha, ori_merged, fg, bg):
        ...
        """
        #print((merged, trimap))
        pred_alpha,aux_alpha = self.backbone(torch.cat((merged, trimap), 1))
        ...
        if self.aux_alpha is not None:
            losses['loss_alpha2'] = self.aux_alpha(aux_alpha, F.interpolate(alpha, size=(aux_alpha.size(2), aux_alpha.size(3)), mode='bilinear', align_corners=True),
                                                  F.interpolate(weight, size=(aux_alpha.size(2), aux_alpha.size(3)), mode='nearest'))
    ...
    def forward_test(self,
        ...
        """
        pred_alpha,_ = self.backbone(torch.cat((merged, trimap), 1))
