...
class IndexNetot(BaseMattor):
    ...
    def __init__(self,
                 ...
                 loss_comp=None,
                 loss_segm=None):
        super(IndexNetot, self).__init__(backbone, None, train_cfg, test_cfg,
                                       pretrained)
        ...
        self.loss_segm = (
            build_loss(loss_segm) if loss_segm is not None else None)
    ...
    def forward_train(self, merged, trimap, meta, alpha, ori_merged, fg, bg,oti_trimap):
        ...
        """
        #print((merged, trimap))
        pred_alpha, mask1 = self.backbone(torch.cat((merged, trimap), 1))
        ...
        if self.loss_comp is not None:
            losses['loss_comp'] = self.loss_comp(pred_alpha, fg, bg,
                                                 ori_merged, weight)
        if self.loss_segm is not None:
            losses['loss_segm'] = self.loss_segm([mask1], oti_trimap)
