...
class RandomAffine(object):
    ...
    def __repr__(self):
        ...
        return repr_str
import cv2
interp_list = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_LANCZOS4]#, cv2.INTER_AREA]
def maybe_random_interp():
    return np.random.choice(interp_list)
@PIPELINES.register_module()
class RIRandomAffine(object):
    """Apply random affine to input images.
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            results[key] = cv2.warpAffine(
                results[key],
                M, (w, h),
                flags=maybe_random_interp() + cv2.WARP_INVERSE_MAP)
