...
@PIPELINES.register_module()
class CropAroundCenterMask(object):
    """Randomly crop the images around unknown area in the center 1/4 images.
    ...
    def __call__(self, results):
        ...
        results['trimap'] = trimap[top:bottom, left:right]
        results['bg'] = bg[top:bottom, left:right]
        results['mask'] = mask[top:bottom, left:right]
        results['crop_bbox'] = (left, top, right, bottom)
        return results
...
class CropU(object):
    ...
    def __call__(self, results):
        ...
        top,bottom, left,right= uindex(trimap)
        results['fg'] = fg[top:bottom, left:right]
        results['alpha'] = alpha[top:bottom, left:right]
        results['trimap'] = trimap[top:bottom, left:right]
        results['bg'] = bg[top:bottom, left:right]
...
@PIPELINES.register_module()
class CropAroundCenterInter(object):
    """Randomly crop the images around unknown area in the center 1/4 images.
    ...
    def __call__(self, results):
        ...
        top = np.random.randint(0, bg_h - crop_h + 1)
        left = np.random.randint(0, bg_w - crop_w + 1)
        bottom = top + crop_h
        right = left + crop_w
        results['bg'] = bg[top:bottom, left:right]
        return results
