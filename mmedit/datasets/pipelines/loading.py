...
interp_list = ['nearest', 'bilinear', 'bicubic', 'lanczos']#,'area']
def maybe_random_interp():
    return np.random.choice(interp_list)
@PIPELINES.register_module()
class RIRandomLoadResizeBg(object):
    """Randomly load a background image and resize it.
    ...
    def __call__(self, results):
        ...
        img_bytes = self.file_client.get(filepath)
        img = mmcv.imfrombytes(img_bytes, flag=self.flag)  # HWC, BGR
        bg = mmcv.imresize(img, (w, h), interpolation=maybe_random_interp())
        results['bg'] = bg
        return results
...
@PIPELINES.register_module()
class RIRandomLoadResizeBgMix(object):
    """Randomly load a background image and resize it.
    ...
    def __init__(self, bg_dir1, bg_dir2,ratio=0.6, io_backend='disk', flag='color', **kwargs):
        self.bg_dir1 = bg_dir1
        self.bg_dir2 = bg_dir2
        self.bg_list1 = list(mmcv.scandir(bg_dir1))
        self.bg_list2 = list(mmcv.scandir(bg_dir2))
        self.ratio = ratio
        self.io_backend = io_backend
        self.flag = flag
        self.kwargs = kwargs
    ...
    def __call__(self, results):
        ...
        if self.file_client is None:
            self.file_client = FileClient(self.io_backend, **self.kwargs)
        h, w = results['fg'].shape[:2]
        low = np.random.random() < self.ratio
        if low:
            idx = np.random.randint(len(self.bg_list1))
            filepath = Path(self.bg_dir1).joinpath(self.bg_list1[idx])
            img_bytes = self.file_client.get(filepath)
            img = mmcv.imfrombytes(img_bytes, flag=self.flag)  # HWC, BGR
            bg = mmcv.imresize(img, (w, h), interpolation=maybe_random_interp())
        else:
            idx = np.random.randint(len(self.bg_list2))
            filepath = Path(self.bg_dir2).joinpath(self.bg_list2[idx])
            img_bytes = self.file_client.get(filepath)
            img = mmcv.imfrombytes(img_bytes, flag=self.flag)  # HWC, BGR
            bg = mmcv.imresize(img, (w, h), interpolation=maybe_random_interp())
        #print(filepath)   
...
@PIPELINES.register_module()
class RIRandomLoadResizeBgInter(object):
    """Randomly load a background image and resize it.
    ...
    def __init__(self, bg_dir,crop_size=512, io_backend='disk', flag='color', **kwargs):
        self.bg_dir = bg_dir
        self.bg_list = list(mmcv.scandir(bg_dir))
        self.io_backend = io_backend
        self.flag = flag
    ...
    def __call__(self, results):
        ...
        img = mmcv.imfrombytes(img_bytes, flag=self.flag)  # HWC, BGR
        fg_min = min(h,w)
        if self.crop_size>=fg_min:
            bg = mmcv.imresize(img, (w, h), interpolation=maybe_random_interp())
            #print('ori_resize')
        else:
            resize_min = np.random.randint(self.crop_size, fg_min+1)
            ratio = resize_min/fg_min
            new_h,new_w = int(h*ratio)+1, int(w*ratio)+1
            #print(img.shape[:2],( new_h,new_w),(h, w))
            bg = mmcv.imresize(img, (new_w, new_h), interpolation=maybe_random_interp())
...
class TrainResize(object):
    ...
    def __call__(self, results):
        ...
        if h*w<2000*2000:
            return results
        ratio = ((2000*2000)/(h*w))**0.5
        h = int(ratio*h)
        w = int(ratio*w)
        #print(ratio,w,h)
        fg = results['fg']
        alpha = results['alpha']
        fg = mmcv.imresize(fg, (w, h), interpolation='bicubic')
        alpha = mmcv.imresize(alpha, (w, h), interpolation='bicubic')
        results['fg'] = fg
        results['alpha'] = alpha
        results['fg_ori_shape'] = fg.shape
        results['alpha_ori_shape'] = alpha.shape
        return results
...
class TestResize(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        h, w = results['merged'].shape[:2]
        results['re_ori_shape'] = results['merged'].shape[:2]
        if h*w<3000*3000:
            return results
        ratio = ((3000*3000)/(h*w))**0.5
        h = int(ratio*h)
        w = int(ratio*w)
        #print(ratio,w,h)
        trimap = results['trimap']
        merged = results['merged']
        trimap = mmcv.imresize(trimap, (w, h), interpolation='nearest')
        merged = mmcv.imresize(merged, (w, h), interpolation='bicubic')
        results['trimap'] = trimap
        results['merged'] = merged
        results['re_ori_shape'] = trimap.shape
        return results

@PIPELINES.register_module()
class GCAResize(object):
    """Randomly load a background image and resize it.
    ...
    def __call__(self, results):
        ...
        fg = results['fg']
        alpha = results['alpha']
        fg = mmcv.imresize(fg, (640, 640), interpolation='bicubic')
        alpha = mmcv.imresize(alpha, (640, 640), interpolation='bicubic')
        results['fg'] = fg
        results['alpha'] = alpha
        results['fg_ori_shape'] = fg.shape
        results['alpha_ori_shape'] = alpha.shape
...
@PIPELINES.register_module()
class LoadImageFromFileCV2(object):
    """Load image from file.
...
@PIPELINES.register_module()
class RIRandomLoadResizeBgMixCV2(object):
    """Randomly load a background image and resize it.
...
@PIPELINES.register_module()
class RIRandomLoadResizeBgCV2(object):
    """Randomly load a background image and resize it.
