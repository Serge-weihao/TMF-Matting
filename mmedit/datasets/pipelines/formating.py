...
class ToTensornb(object):
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            results[key] = to_tensor(results[key]).type(non_blocking=True)
        return results
...
class ImageToTensor(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        #print('before',results['mask'].shape)
        for key in self.keys:
        ...
        #print('after',results['mask'].shape)
        return results
...
class Collect(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        #print('before collect',results['mask'].shape)
        data = {}
        img_meta = {}
        ...
        #print('after collect',results['mask'].shape)
        return data
...
class TransTrimap(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        trimap_im = results['trimap'].squeeze()
        h,w=trimap_im.shape
        
        trimap = np.zeros((h, w, 2))
        trimap[trimap_im == 255, 1] = 1
        trimap[trimap_im == 0, 0] = 1
        trimap_transformed = trimap_transform(trimap)
        #print(trimap_transformed.shape)
        results['trimap_transformed'] = torch.from_numpy(trimap_transformed)#.transpose(1,2,0)#.permute(2, 0, 1)
        #print(results['trimap_transformed'].shape)
        return results
...
def trimap_transform(trimap, L = 320):
    ...
    for k in range(2):
        #print(dt(1 - trimap[:, :, k]).max())
        dt_mask = -np.clip(dt(1 - trimap[:, :, k]),0,3000)**2
